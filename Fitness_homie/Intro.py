...
def workout_info():
    fileDir = os.path.dirname(os.path.realpath("__file__"))
    names = ["Chest_day_workout.txt", "Arm_day.txt", "Back_day.txt", "Leg_day.txt", "Core.txt", "Workout_videos.txt"]
    ...
    for i in range(0, len(names)):
        ...
        if fileexist  == False:
            ...
            admin.close()
    admin1 = open(names[0], "r")
    admin1value = admin1.read().splitlines()
    admin1.close()
    
    admin2 = open(names[1], "r")
    admin2value = admin2.read().splitlines()
    admin2.close()
    
    admin3 = open(names[2], "r")
    admin3value = admin3.read().splitlines()
    admin3.close()

    admin4 = open(names[3], "r")
    admin4value = admin4.read().splitlines()
    admin4.close()

    admin5 = open(names[4], "r")
    admin5value = admin5.read().splitlines()
    admin5.close()

    admin6 = open(names[5], "r")
    admin6value = admin6.read().splitlines()
    admin6.close()
    
    length1 = len(admin1value)
    length2 = len(admin2value)
    length3 = len(admin3value)
    length4 = len(admin4value)
    length5 = len(admin5value)
    length6 = len(admin6value)
    return render_template("workoutinformation.html", chest = admin1value, length1 = length1, length2 = length2, length3 = length3,
                           length4 = length4, arm = admin2value, back =admin3value, leg = admin4value, length5 = length5, more_info = admin5value,
